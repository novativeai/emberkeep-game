...
- SHARING THIS CHECKOUT with another agent? `pnpm verify:lane <name>` — it derives all
  four vars (port, `dist`, output dir, shots dir) from one lane id, so two runs cannot
  silently test each other's build (one `dist`, one port 4173, and `reuseExistingServer`
  makes the second run reuse the first's server — every failure then lands somewhere
  else, in code neither side touched). `--print` shows the lane without running it.
  By hand, all four or nothing:
  `EMBERKEEP_PREVIEW_PORT=4273 EMBERKEEP_DIST=dist-verify EMBERKEEP_E2E_OUT=test-results-verify EMBERKEEP_E2E_SHOTS=tests/e2e/shots-verify pnpm verify`
...
- `docs/worlds.md` — the three authored worlds (nb2 / roothold / borealis): what each
  holds, how they link, and the measured per-grid cell loss. Read before touching
  world switching, editor grids, or the lattice.
...
- XP (`LEVEL_XP = [0, 60, 220]`) is tuned so the tutorial's EXACT 60 XP lands
  Level 2 on the scripted `levelup` beat (wakes zone 2), and Level 3 (220) is
  the Chapter One FINALE — golden hatch + south-terrace glimpse + Cindra's line
  + chapter card, choreographed off the shared `FINALE` timeline in Constants.
  The array deliberately ends at level 3; the XP bar reads "Chapter One
  complete" at the cap.
...
and the Title Play button's position. Also `BeyondDemoPanel.NAME` — Phaser TimerEvents
advance by the CLAMPED frame delta, and this canvas runs at 1-2 fps under SwiftShader
(measured: a `delayedCall(5000)` had not fired after 60s), so the e2e waits for the
finale panel by name instead of sleeping through its 12.6s timeline. Never re-time that
beat with a fixed sleep.
