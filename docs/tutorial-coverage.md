...
| A House is commissioned, once — and to its RANK | the chooser, on the House raised at `plank_merge` | `house_commission` | the chooser is suppressed by `allow.commission` for the whole tutorial EXCEPT this beat, so it can never land unheralded on `house_skip`. Placed deliberately AFTER `resin_pocket`: only what is in the satchel can be commissioned, and the beat before it puts the right thing there. The line also names the rank rule — a House works tier-1 pieces, a Manor takes tier 2 as well (`produceMaxTier`); the Resin Bead it points at is tier 1, so the taught case is the good case |
...
| **Board hygiene** — generators work anywhere; carry them off the crowded hearth | post-tutorial: generators only become draggable when the allow-lists lift | self-teaching on the first free-play drag | the dedicated `move`-gated beat (`board_room`) was retired: the stump now spawns out on the open field ([7,2]) and the camera follows every pointer there, so the habit it staged — making room — never arises mid-script. Dragging a generator credits nothing and blocks nothing, so discovery is safe |
