...
| Tapping somebody reads them | any character or dragon | `eleanor_hearts` | gates on the tap itself, so the verb is performed rather than described. `allow.status` debuts the readout here and LATCHES — the dragon beats later reuse the same surface |
...
| A worked tree seeds its replacement | the Ancient Tree's 10th Cut Wood | `tree_grain` | the Grain drops from the tree the player is already watching. The bonus-yield rule was named one beat earlier on the Emberberry patch, so this is the SAME lesson with a second example — which is why the beat only has to point at the roots |
| Fir: 3 Grains → a Small Fir Tree | `tree_grain`'s three Grains | `grain_merge` | an ordinary merge on a piece the tree just gave; no new verb |
| A tree the player grew, that gives like the old one | the Small Firs | `fir_grow` | 3 Small Firs → a Fir Tree with the SAME produces + bonus as the Ancient Tree. This is the beat that closes the loop — the isle's supply stops being finite scenery and becomes something the player farms |
...
| Resin — the tree's other gift | the old trunk, at `resin_find` | `resin_find` | 3 beads spawned at the Ancient Tree, the same landmark the Fir Grains came off. Taught end to end inside the tutorial (beads → Lump → Hearth Cake), which is what took it out of `HIDDEN_CHAINS`: both its Cookbook rows are discovered in the lesson, so it is never a row the chapter cannot finish |
