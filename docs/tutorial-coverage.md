...
| **The Dragon Codex** — the dragon-head button above the Cookbook | appears with a pop the moment the first dragon is NAMED (`dragon:named`) | self-teaching on first tap | same gate contract as the Cookbook button beside it: visible mid-tutorial, tappable once the script hands the game over. The panel is its own lesson — face card → lore card (story, personality, ability, well-fed cycles) → Evolution page (adult silhouette + condition) |
