...
| A building is commissioned, once | the chooser, on the Manor raised at `house_merge` | `house_commission` | the chooser is suppressed by `allow.commission` for the whole tutorial EXCEPT this beat, so it can never land unheralded on `house_skip`. Placed deliberately AFTER `resin_pocket` (only what is in the satchel can be commissioned, and the beat before puts the right thing there) and after `house_merge` (the building that takes the order is the one the player just raised). `chooseProduce` is on `lumber:4` as well as `lumber:3` for exactly that reason — without it the merge would hand the player a Manor that cannot be told anything |
| A commission is a commitment, not a menu | the chooser's Yes/No | `house_commission` | write-once is invisible until you try to change it, so the beat spends its line on WHAT to pick rather than on the price of picking |
...
| The Emporium sells Warmth | ⚡+ button, frame 1 | `buy_energy` | first Warmth gift is free |
...
| Unlimited Warmth (∞ label under the gauge) | the Warmth gauge, only after an Unlimited Warmth grant (built; no pack sells it since 2026-09-10 — the Hearth Hoard now carries 1,250 Warmth) | `buy_energy` | not scripted: the confirm dialog and purchase banner state it and the label counts it down; building skips (the `house_skip` lesson, whatever the House was commissioned to) keep their price, so no beat's Warmth drop disappears; `gem_harvest`'s "costs 1 Warmth" is inaccurate only while a purchased benefit is running, and its gate still fires |
...
| Two Houses make a Manor | `house_merge` (a second House is spawned there) | `house_merge` | the ONE merge in the chain that is not three-of-a-kind — `lumber:3` carries `merge: {group: 2}` — so it is taught with the hand rather than left to a player who has been shown "three makes one" nine times. It runs BEFORE `house_commission` on purpose: the commission panel is the payoff for having built the bigger building, not a thing the starter House does first and the Manor repeats. `twoHouses` remains as the post-tutorial contextual hint for a player who meets a second House later |
