 ...
 *
 * The sheet is played where the LIVE three.js gem is declined (iOS, the `low`
 * profile). It carries its own `anchor` and `itemScale` because the packer trims
 * the source canvas symmetrically and may rescale it — the `anchors.json` entry
 * and `ITEM_SCALE` that fit the untrimmed 803x902 texture would seat it wrong,
 * so the numbers that describe the pixels ship WITH the pixels.
 */
