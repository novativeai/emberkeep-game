  ...
  private buildOutline(scene: Phaser.Scene): void {
    ...
    for (const ls of this.layers) {
      ...
      const twin = scene.add
        // `__BASE` by name: a rig layer is a standalone texture, so it is also
        // the default frame — but the shader's uv remap REQUIRES it, and asking
        // for it by name is what stops a layer that ever arrives packed in an
        // atlas from silently losing its outline. See BASE_FRAME in ./SpriteInk.
