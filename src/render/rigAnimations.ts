...
export const PRESETS: PresetDef[] = [
  {
    ...
    fn: (t) => {
      const p = emptyPose();
      p.root.sy = 1 + 0.018 * Math.sin(t * TAU * 0.5);
      p.root.dy = -3 * Math.sin(t * TAU * 0.5);
      p.partDeg['head'] = 3.0 * Math.sin(t * TAU * 0.33);
  ...
  },
  {
    ...
    fn: (t, K) => {
      ...
      p.root.dy = -34 * hop;
      p.root.sy = 1 + 0.05 * hop;
      p.root.sx = 1 - 0.03 * hop;
      ...
      p.partDeg['head'] = 6 * Math.sin(t * Math.PI * hopF) - 4;
