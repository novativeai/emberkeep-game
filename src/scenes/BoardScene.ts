...
/**
 * Chains that have a live animal on the board.
 *
 * This replaced `DRAGON_RIGS`, which used to answer two unrelated questions at
 * once: where to fetch a pin rig, and whether a chain is an animal. The rigs
 * are gone — every dragon is an Align-Studio clip sequence now — so only the
 * second question is left, and the clip roster is the only thing that knows the
 * answer. Derived rather than listed: a breed that gets clips is a dragon here
 * the moment it is registered, with no second list to forget.
 */
const DRAGON_CHAINS = dragonChainsOf();
...
export class BoardScene extends Phaser.Scene {
  ...
  /** The Elder's Align-Studio clip sprite. When her wan-generated clip set is
   *  resident she is CLIP-COMPLETE, exactly like the Emporium breeds: the
   *  sprite IS the whole animal and no RigPlayer is mounted at all. The rig
   *  path below survives as the fallback for a session whose sheets failed. */
  private altarElderClip?: Phaser.GameObjects.Sprite;
  /** Where the clip Elder is in her hover arc ('ground' when idle/roaring). */
  ...
  private ensureDragonClips(chain: string, tier: number): void {
    ...
    this.load.once(Phaser.Loader.Events.COMPLETE, () => {
      ...
      for (const sprite of this.itemSprites.values()) {
        ...
        // Anything already live is already wearing clips — there is no rig to
        // upgrade from since the pin rigs were deleted.
   ...
   *  which goes invisible but stays interactive/draggable. Every dragon is a
   *  clip sequence now, so this returns false only when the sheets have not
   *  landed yet — the caller falls back to the static sprite and the loader's
   *  COMPLETE handler re-dresses it. */
  private attachDragon(host: BoardItem, intro: boolean): boolean {
    if (!this.clipComplete(host.chain, host.tier)) return false;
    ...
    // A dragon is a calm ADULT when it carries a fly clip — the adults are the
    // ones that were given flight. `tier >= 4` stopped being a usable proxy
    // when frost/storm became their own 3-tier chains: their adults sit at tier
    // 3, and the ember adult stays at 4, so no one tier number means "grown"
    // across chains any more.
    const clipArt = this.clipCharacterFor(host.chain, host.tier);
    const calm = clipArt !== null && clipFor(clipArt, 'fly') !== null;
    host.setArtVisible(false); // host is now just the invisible hit-target + bob anchor
    // Ground shadow proportional to the animal (666px source pieces × scale).
    ...
    if (intro) {
      // The newborn roars its arrival — the ingested roar clip, the same
      // bellow as the hungry cadence. Whatever ends up on screen is what fades
      // in; a breed whose sheets are still in flight simply arrives at rest.
  ...
  /** One hungry roar — the ingested roar clip. Purely a mood; nothing in the
   *  economy hears it. Every breed has one, so a dragon that cannot play it is
   *  simply one whose sheets have not landed, and silence is the right answer. */
  private roarOnce(ld: LiveDragon): void {
    // The sparks and the shout land either way: they are the part the PLAYER
    // reads, and a dragon whose sheets are still in flight should still be
    // visibly hungry.
  ...
  private dragonOverlay(ld: LiveDragon, key: string): Phaser.GameObjects.Sprite {
    if (!ld.clipOverlay) {
      ...
      attachSpriteInk(this, ld.clipOverlay, {
        units: this.clipOutlineUnits(ld)
      });
    }
  ...
  private dragonIdle(ld: LiveDragon): void {
    ...
    // Every breed has an idle clip; reaching here means its sheets are not
    // resident yet, and the host's static art is already showing.
  }
  ...
  private applyDragonMood(itemId: number, mood: 'awake' | 'hungry' | 'asleep'): void {
    ...
    this.dragonIdle(ld); // back onto the idle loop (alpha reset by dress)
    ld.mode = 'idle';
    ld.remainMs = DRAGON_WAKE_MS;
  ...
  private showAltarEgg(ceremony: boolean): void {
    if (!this.altarEgg && !this.altarElderClip && !this.altarElderFallback) {
   ...
   *  (the wan-generated idle/roar/fly), else the live rig, else a gold-tinted
   *  stand-in (upgraded automatically when clips or rig arrive). */
  private showAltarElder(): void {
    if (this.altarElderClip) return;
    const p = this.altarPoint();
    ...
    // so clip.scale lands her exactly where the rig stood.
  ...
  /**
   * The Elder's clip set (character-anims `golden_egg:2` → golden_adult), with
   * the named clip's Phaser anim — and, for a phased clip, its segment anims —
   * registered. Null when the clips are not pushed or the sheet is not
   * resident, which is what sends showAltarElder down the rig path.
   */
  private elderClip(clipId: string): { clip: CharacterClip; key: string } | null {
    ...
    for (const [seg, [start, end]] of Object.entries(clip.segments ?? {})) {
      const sk = this.segKey(key, seg);
      if (this.anims.exists(sk)) continue;
      this.anims.create({
        key: sk,
        frames: this.anims.generateFrameNumbers(key, { start, end: end - 1 }),
        frameRate: clip.fps,
        repeat: seg === 'loop' ? -1 : 0
      });
    }
  ...
  /** Bind a clip to the Elder's sprite and dress it NOW — same discipline as
   *  dressOverlay: a clip switch never renders one frame in the previous
   *  clip's transform. She faces right (the flip), watching over the isle. */
  private dressElder(c: { clip: CharacterClip; key: string }): void {
   ...
   * an airborne Elder through her landing segment first (never a touchdown
   * still mid-cruise); 'hover' ramps takeoff → cruise loop; 'announce' is the
   * awakening — the roar clip once, jaws wide and wings flared, then straight
   * into the hover the finale's roll keeps her in for introCelebrateMs.
   */
  private playElder(action: 'idle' | 'hover' | 'announce'): void {
    ...
    if (action === 'hover') {
      ...
      // No phased fly pushed: the idle IS the celebrate, like the clip babies.
      action = 'idle';
    }
    // idle — via the wing-fold when she is airborne.
  ...
  private awakenAltarElder(): void {
    ...
    this.tweens.add({
      ...
      onComplete: () => {
        ...
        this.showAltarElder();
        if (this.altarElderClip) {
          this.playElder('announce'); // clip path: the roar clip IS the announcement
  ...
  private ensureAltarZone(): void {
    ...
    this.altarZone.on('pointerup', (pointer: Phaser.Input.Pointer) => {
      ...
      if (this.altarElderClip || this.altarElderFallback) this.communeWithElder();
  ...
  private onCharacterTapped(characterId: string, sprite: Phaser.GameObjects.Image): void {
    ...
    // She is holding the favour now — the tutorial pointer moves on to what she
    // must be aimed at (TutorialArrowThenConfig).
    this.ctx.bus.emit('character:armed', { characterId });
  ...
  /**
   * A standee's tap area, seated for the current interaction state.
   *
   * Her WHOLE body while a tap on her is the thing the game is asking for — a
   * held give (every tap is aimed at a recipient, nothing on the board left to
   * protect), or a tutorial step whose allow is the character verb
   * (`eleanor_helps` points its arrow at her top-centre, and the narrow rect
   * made exactly that spot dead: "tap me, then tap the House" refused the tap
   * on her it had just asked for). Her lower body otherwise, so her
   * two-tile-tall frame cannot swallow taps meant for the cells behind her.
   *
   ...
   */
  private applyCharacterHitAreas(): void {
    const whole = !!this.pendingGive || (!this.tutorialDone && this.allow.character);
   ...
   * the piece out of the bag ONLY if it actually moved.
   */
  private deliverGiveTo(target: { kind: 'character'; id: string } | { kind: 'dragon'; id: number }): void {
    ...
    if (!taken) {
      // A refusal ENDS the gesture — visibly, back to the satchel. It used to
      // leave the give armed ("tap somebody else"), but a held give claims
      // every tap on a character (the give-first routing in onCharacterTapped),
      // so a quiet refusal left an invisible state in which Eleanor could never
      // be tapped to arm her CAST again. Give state exists only between the
      // bag's Give press and one delivery attempt; outside it, her tap always
      // does what her tap does.
      this.cancelGive();
      return;
    }
   ...
   */
  /**
   * The Ember Gate flight, with the WAKING as its opening beat.
   *
   * A sleeping dragon flown at the portal is a curled painting on a bezier: the
   * rig is hidden behind the sleep art, none of the flight clips are mounted,
   * and it arrives at the door still asleep — which is what left it stuck
   * there and drifting back. So the sleep ends FIRST. `keepAwake` covers the
   * whole cinematic (it outranks the nap, the night and a shift-rest alike),
   * the uncurl plays on the ordinary mood path, and only once it is on its
   * feet — GATE_FLIGHT.wakeLeadMs later — does the flight begin.
   */
      ...
      (pointer: Phaser.Input.Pointer, obj: Phaser.GameObjects.GameObject) => {
        ...
        if (
          ...
          const tgt = [...this.itemSprites.values()].find((s) => {
            ...
            if (!cfg || cfg.tappable !== false || DRAGON_CHAINS.has(s.chain)) return false;
  ...
  private tryFeedDrop(obj: BoardItem, to: TilePos, pointer: Phaser.Input.Pointer): boolean {
    ...
    const kind = this.ctx.systems.dragonLife.sleepKindOf(target.itemId);
    if (kind) {
  ...
  private onItemTapped(sprite: BoardItem): void {
    ...
    if (DRAGON_CHAINS.has(item.chain) && isGenerator && (this.tutorialDone || this.allow.dragonWork)) {
    ...
    this.ctx.bus.emit('item:tapped', { itemId: sprite.itemId });
    if (isGenerator && !DRAGON_CHAINS.has(item.chain)) this.sendDragonFlourish(sprite);
  }
  ...
  private sendDragonFlourish(plant: BoardItem): void {
    ...
    //
    // The journey waits for the WINGS. Takeoff plays in place; only once the
    // cruise loop is running does the sprite leave its tile, and it does not
    // touch down again until it is home — one takeoff, one flight out and
    // back, one landing.
    const journey = (): void => {
      this.tweens.add({
      targets: dragon,
  ...
  private startDragonWork(
    ...
    const house =
      ...
      [...this.itemSprites.values()]
        .filter((s) => {
          const cfg = this.generatorConfigFor(s.chain, s.tier);
          return cfg?.tappable === false && !DRAGON_CHAINS.has(s.chain) && s.itemId !== sprite.itemId;
        })
        .sort(
          (a, b) =>
  ...
  private subscribe(): void {
    ...
    this.offBus.push(
      ...
      bus.on('tutorial:step', (step) => {
        ...
        // A step that asks for HER tap widens her tap area to her whole body —
        // its arrow points at her head, which the lower-body rect never covered.
        this.applyCharacterHitAreas();
