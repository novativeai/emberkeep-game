...
/**
 * Item card: name and tier dots — "tell me about this", opened by a HOLD.
 *
 * It used to carry a Sell button, and no longer does: selling moved into the
 * Bag, so nothing on the board can be sold and this card has no destructive
 * action left on it. That is the point — a hold now only ever tells you
 * something, and the one irreversible verb sits behind pocketing the piece
 * first (EconomySystem.sellFromBag).
 *
 * Opens above the held item; any outside tap closes it (handled by UIScene).
 */
export class Tooltip extends Phaser.GameObjects.Container {
  ...
  constructor(scene: Phaser.Scene, private chains: ChainsData) {
    super(scene, 0, 0);
    ...
    this.add([this.bg, this.nameText, ...this.dots]);
