...
describe('the Borealis roster', () => {
  ...
  // NOT PORTED: main's "the new farms are seeded, live, and every Cookbook row
  // is finishable". It asserts that runestone/emberdram/manastone stand on a
  // board somewhere, which is a claim about MAIN'S map. Ours is generated by
  // the Map Editor and seeds none of the five northern farms yet — they are
  // wired end to end (chains, generators, diet, recipient locks, art) and
  // placed nowhere. That is a data gap owned by the map, not by this code, so
  // asserting it here would only fail on somebody else's file. It comes back
  // the day the export puts them down.
});
