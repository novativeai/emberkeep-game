...
describe('the crystal goes home on load', () => {
  ...
  it('takes the plinth back from a squatter without destroying it', () => {
    // OUR RULE, and it is deliberately not main's. Main left the landmark where
    // it strayed whenever the plinth was occupied; this tree takes the cell back
    // unconditionally, because the piece squatting on a fixture's cell only got
    // there by the same accident, and a plinth is not a legal drop target — so
    // the polite version stranded the landmark for ever with no way for anyone
    // to notice. What must still hold is the half that is not about politeness:
    // the squatter is an earned piece, so it is re-seated or banked, never
    // deleted, and no two pieces end on one cell.
    const storage = new MemoryStorage();
    const ctx = ctxWith(storage);
    ctx.beginRun();
    const stray = freeCellAwayFromPlinth(ctx);
    ...
    expect([healed.col, healed.row]).toEqual(plinth); // the landmark comes home
    const onBoard = items.find((i) => i.id === squatter.id);
    const banked = fresh.state.bag.some((b) => b.chain === 'lumber' && b.count > 0);
    expect(Boolean(onBoard) || banked, 'the squatter is kept, not deleted').toBe(true);
    if (onBoard) expect([onBoard.col, onBoard.row]).not.toEqual(plinth);
    expect(superposed(fresh)).toEqual([]);
  });
});
