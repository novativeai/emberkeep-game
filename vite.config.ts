...
const animAlignEndpoint = (): Plugin => ({
  ...
  configureServer(server: ViteDevServer) {
    server.middlewares.use('/__animalign', (req, res) => {
      // No CORS headers here: `devApiGuard` leads the plugins array and stamps
      // them for an origin we actually serve. Main set `*`, which this tree
      // deliberately refuses — a dev API writes source files.
      res.setHeader('Content-Type', 'application/json');
      if (req.method === 'OPTIONS') {
        res.statusCode = 204;
        res.end();
